...
from domain.device.crete import crete_dpe

class creteconfig(object):
    ...
    def __getlocationPort(self):
        ...
        try:
            ...
            pass
        try:
            loc2 = int(BuiltIn().get_variable_value('${dut1port102}'))
        except:
            pass
        return loc1,loc2
    
    def getClassifySuiteConfig(self):
        loc1,loc2 =self.__getlocationPort()
        value = {                      
            'crete_iport_rx':{
                "tbl_name": "crete_iport_rx",
                "source_port": loc1,  #最多支持4个物理口,从4起为环回原因
                "overlay": 0,
                "opcode": 2, # classify hash
                #"default_destination": 1000,
                "group_index": loc1+1, 
                "action_type": 0,
                #"tid": 57
            },                       
            'crete_default_action' : {
                ...
                "table": {
                    "em_replace_dmac": "12:34:56:78:55:66",
                    "em_opcode": 3,
                    "em_destination_valid": 1,
		            "em_action_type": 0,
                    "em_destination": loc2<<6 if loc2 else 0, # 0口
                }
            }

        }
        value = crete_dpe.crete_config_check(value)
        return value
    ...
    def __getClassifyValue(self,pkttype,layer,mode=2,ilist=None):
        loc1,_ = self.__getlocationPort()
        pkttype = pkttype.lower()
        pkt = crete_dpe.getPacket(pkttype)
        
        #这里MAC写死是因为代码hash查找还有问题
        pkt[Ether].dst = "00:10:20:30:40:50"
             
        pktchk = pkt.copy()
        pktchk[Ether].dst = "12:34:56:78:55:66"
        #pktchk = pktchk/20*'x'
        value = {
            'crete_classify_hash' :{
                "tbl_name": "crete_classify_hash",
                "index": 0,
                "key_outmost_dmac": pkt[Ether].dst, #最外层DMAC 地址,或者vlan，vlan先不考虑
                "key_group_index": loc1+1, #直接使用port 索引,
                "action_type": 0,
                "opcode": 2,
                "group_index": loc1+1, #直接使用port 索引,,
            },
            'crete_classify_cam' :{
                ...
                "opcode": 1,  #default action
                "tid": 0,
                "action_type": 0,
                "destination_valid": 1,
                "default_action_index": self.default_action,    
            },
            'register':{
                "group":[
                    {
        ...
        #pkttunlnum = crete_dpe.getTunlnumByPkttype(pkttype)
        info = crete_dpe.paserPacket(pkt)
    
        value['crete_classify_cam'],ilist = crete_dpe.setClassifyByPkt(value['crete_classify_cam'],info,layer,mode,ilist)
        value['register'] = crete_dpe.setClassifyRegisterByConf(value['register'],info,layer,mode,ilist)
        #value['input_port_rx'] = crete_dpe.setInputPortRxByTunnel(value['input_port_rx'],info)
    ...
    def getClassifyValue(self,pktname,layer,mode=2,ilist=None):
    ...
    def getProfileSuiteConfig(self):
        ...
        profileadd = []
        #self.proflielist = profileadd + crete_dpe.getProfileDefaultTemplates('default')
        for t in self.proflielist:
            p = self.createProfile(t)
            value['profile']['group'].append(p)
        value = crete_dpe.corsica_config_check(value)
        return value
    ...
    def createProfile(self,t):
        profile = {
            "key_group_index": self.groupid, 
            "key_profile_id": self.profileid,
            "mask_group_index": "0xFF",
            "mask_profile_id": "0xFF",
            "opcode": 1,
            "default_destination": self.default_action,
        }
        crete_dpe.setProfileTemplate(profile,t)
        return profile
    
    def getProfileValue(self,pktname):
        pktname = pktname.lower()
        pkt = crete_dpe.getPacket(pktname)
        profile = None
        ...
        #IP分片不在不是first
        pktchk = crete_dpe.parsePktIpFragment(pktchk)
                ...
                "group":[
                    {
                         ...
                         "key" : {
                            "INPUT_PORT_RX_SRC_PORT" :  loc1,  #最多支持4个物理口,从4起为环回原因
                    ...
                    },
                    {
                         "tbl_name": "input_port_rx",
                         "key" : {
                            "INPUT_PORT_RX_SRC_PORT" :  loc1,  #8是0口
    ...
    def getEmValue(self,pktname,keymask,keylen=None,action=None):
        ...
        if keymask :
            template,specConf = crete_dpe.setKeyTemplate(template0,keymask)
        else:
            template,_ = crete_dpe.setKeyTemplate(template0,pkt)


        if keylen == None:
            iemkeypara = crete_dpe.getEmKeyTpid('default','768bits')
        else:
            keylendict = crete_dpe.tansStrToDict(keylen)
            for k,v in keylendict.items():
                iemkeypara = crete_dpe.getEmKeyTpid(v['type'],k)
                           
        profileemkeylen = iemkeypara[0]
        tbl_id = iemkeypara[1]
        emkeylen = iemkeypara[2]
             
        #ikey,_ = crete_dpe.getEmKey(pkt,template,keylen=keylen,config=dict())
        ikey,_ = crete_dpe.getEmKey(pkt,template,iemkeypara,specConf)
        value = {
            ...
            'key_mask' :{
                ...
                "table": crete_dpe.getKeyMask(keymask)
            },
            'em_table': {
                    "tbl_id":tbl_id,                
                    "key_len":emkeylen,               
    ...
    def getEmActionSuiteConfig(self):
        ...
        value['register'] = crete_dpe.setDscp2PriReg(value['register'])
    ...
    def getEmActionValue(self,pktname,keymask,keylen=None,atype=None,action=None):
        ...
        if keymask :
            template,_ = crete_dpe.setKeyTemplate(template0,keymask)
        else:
            template,_ = crete_dpe.setKeyTemplate(template0,pkt)
        ...
        #ikey,_ = crete_dpe.getEmKey(pkt,template,keylen=keylen,config=dict())
        ikey,_ = crete_dpe.getEmKey(pkt,template,iemkeypara,config=dict())
        value = {
            'key_template' : template0,           
            'profile' :{             
                "key": {
        ...
        em = value['em_table']
        regconf = value['register']
        tunnel_encap = value['tunnel_encap']
        if atype == 'action':
            em['table'].pop("REPLACE_SMAC")
            crete_dpe.setDefaultAction(em,action)
            crete_dpe.setTunnelEncapCfgTable(tunnel_encap,action)
            pktchk = crete_dpe.getDefaultActionPacket(pkt,em,regconf)
        elif atype == 'drop':
    ...
    def getInportActionValue(self,pkttype,atype,action):
        loc1 =self.__getlocationPort()
        pkt = crete_dpe.getBasePkt(pkttype)
        #只修改最外层MAC
        value = {
            'default_action' : {
        ...
        if atype == 'action':
            crete_dpe.setDefaultAction(defaultaction,action)
            pktchk = crete_dpe.getDefaultActionPacket(pkt,defaultaction)
        elif atype == 'drop':
...
if __name__ == '__main__':
    ...
    #print(crete_dpe.saveJson(c.getClassifyIpv4Value(mode=-1,ilist=['ETH_TYPE'])['corsica'],"ClassifyIpv4_vlan_"))
    print(crete_dpe.saveJson(c.getClassifyValue("ipv4_qq",mode=-1,ilist='OUT_VLAN_ID')['corsica'],"ClassifyIpv4_vlan_"))
    print(crete_dpe.saveJson(c.getClassifyValue("ipv4_qq",mode=-1,ilist='OUT_VLAN_ID,OUT_VLAN_TPID')['corsica'],"ClassifyIpv4_vlan_"))
    for i in range(10):
        print(crete_dpe.saveJson(c.getClassifyIpv4Value(i)['corsica'],"ClassifyIpv4_%d_"%i))
    
    print("======getClassifyIpv4Value - Save========")
    for i in range(10):
        print(crete_dpe.saveJson(c.getClassifyIpv6Value(i)['corsica'],"ClassifyIpv6_%d_"%i))

    print("======getClassifyIpv4Value - Save========")
    for i in range(10):
        print(crete_dpe.saveJson(c.getClassifyIpv4VxlanValue(i)['corsica'],"ClassifyIpv6_%d_"%i))
