...
class corsica_dpe (object):
    default_mask_json = {
        "classify" : {
            ...
            "CLASSIFY1_OUT_TUNNEL_TYPE": (1<<5)-1,
            "CLASSIFY1_OUT_L5P5_HEAD_TYPE": (1<<3)-1,
            "CLASSIFY1_INNER_TUNNEL_TYPE": (1<<5)-1,
    ...
    def value_check(k,v,default=1):
        ...
        if type(default) == int:
            if type(v) == str:
                if 'INPUT_PORT_RX_SRC_PORT' == k :
                    if 'p' in v :
                        return int(v[1:])+8
                if 'p' in v : #端口号 p103:100
                    if ":" in v: #指定了q
                        p,q=v[1:].split(':',maxsplit=1)
    ...
    def createVlanData(info):
        ...
        vid = info.vlan
        mask = 0x07   #add时不关心
        val = struct.pack(">BH", (tpid<<5) + mask, (pri << 13) + (cfi << 12) +vid) 
        return val       
    ...
    def createData(config):
        ...
        for k,l,t in keylist:
            ...
            if "TOS_UPDATE" == k:
                ...
                config[k] = info[0]
                vlan = info[1]
                val = corsica_dpe.createVlanData(vlan)
                l = 24
        
                if vlan.type == 0x8100:
                    vlan1 = info[1].payload 
                    val += corsica_dpe.createVlanData(vlan1) 
                    l += 24  
            elif "L2_FLAG" == k:  
                if type(info) == list:
                    config[k] = info[0]
    ...
    def getPacket(itype,pktlen=0,padlen=0):
        ...
        if  ot:
            ...
            if k+'vlan' in info:
                ot[Dot1Q].vlan = 100
            if k+'vlan1' in info:
                ot[Dot1Q].payload[Dot1Q].vlan = 101
            if k+'ipv4' in info:
                ot[IP].fields.setdefault('src','192.188.1.222')
                ot[IP].fields.setdefault('dst','192.188.1.210')
            if k+'ipv6' in info:
                ot[IPv6].fields.setdefault('src','192::1')
                ot[IPv6].fields.setdefault('dst','192::2')
            if k+'udp' in info:
        ...
        if tl :
            ...
            if k+'vlan' in info:
                tl[Dot1Q].vlan = 200
            if k+'vlan1' in info:
                tl[Dot1Q].payload[Dot1Q].vlan = 201
            if k+'ipv4' in info:
                tl[IP].fields.setdefault('src','100.22.1.222')
                tl[IP].fields.setdefault('dst','100.22.1.210')
            if k+'ipv6' in info:
                tl[IPv6].fields.setdefault('src','100::1')
                tl[IPv6].fields.setdefault('dst','100::2')
        if inner :
            ...
            if 'vlan1' in info:
                tl[Dot1Q].payload[Dot1Q].vlan = 201
            if 'ipv4' in info:
                tl[IP].fields.setdefault('src','200.22.1.222')
                tl[IP].fields.setdefault('dst','200.23.1.210')
            if k+'ipv6' in info:
                tl[IPv6].fields.setdefault('src','300::1')
                tl[IPv6].fields.setdefault('dst','300::2')
        if Ether not in pkt:
            pkt = Ether()/pkt
        return pkt
    ...
    def setProfileTemplate(profile=None,itype=None):
        ...
        if ot != None:
            ...
            if ottype:
                ...
                if key["PROFILE_OT_TUNNEL_TYPE"] == 'vxlan' or key["PROFILE_OT_TUNNEL_TYPE"] == 'vxlan-ali':
                    key["PROFILE_OT_TUNNEL_FLAGS"] = 0x08
                elif key["PROFILE_OT_TUNNEL_TYPE"] == 'vxlan-gpe':
                    key["PROFILE_OT_TUNNEL_FLAGS"] = 0x0c    
                elif key["PROFILE_OT_TUNNEL_TYPE"] == 'geneve':
                    key["PROFILE_OT_TUNNEL_FLAGS"] = 0x0c 
    ...
    @staticmethod
    def getEmActionUpdateFieldRegv(config=None, type='dst',info=None): 
        conf = config[type]
        layer = conf.get('layer','ot_l3')
        field = conf.get('field','dst')
        offset = conf.get('offset','0')
        fieldval = info[layer].getfieldval(field)
        width = conf['width']
        ...
        #模板定义 
        # bit[3:0]:域段，bit[9:4]:长度，bit[17:10]:offset，bit[21:18]:mask模板号，
        # bit[22:22]:use_mask,bit[24:23]:op_code, bit[25:25]:source_data_mode, 共26bits      
        
        segv = corsica_dpe.getEmActionTypeid('updfield_layer', layer)
        ...
        if width % 4 !=0 :
            ...
            length = width/4
            use_mask = 0
        offset = offset%4 #必须是4的整数倍
        op_code = corsica_dpe.getEmActionTypeid('updfield_opcode',config.get('opcode','set'))
        
        source_data_mode = config['src']['src_mode']  
        data = source_data_mode << 24 + op_code << 23 + use_mask << 21 + fieldlenmask << 20 + offset << 16 + length << 8 + segv
        return data         
           
    @staticmethod
    def getEmActionUpdateField(config=None,info=None):              
        dstl = config['dst'].get('layer','ot_l3')
        dstf = config['dst'].get('field','dst')
        dstoff = config['dst'].get('offset','0')
        dstfv = info[dstl].getfieldval(dstf)
        
        #计算目的字段的模板寄存器       
        if v['src']['src_mode'] == 'data':
            data = v['src'].get('data',random.randint(0,0x12345678))            
        else:
            #计算域段
            srcl = config['src'].get('layer','ot_l3')
            srcf = config['src'].get('field','dst')
            srcoff = config['src'].get('offset','0')
            srcfv = info[dstl].getfieldval(dstf)
            data = corsica_dpe.getEmActionUpdateFieldRegv(config)
    ...
    @staticmethod
    def getDefaultActionPacket(pkt,config,layer="ot"):
        ...
        for k,v in config.items():
            ...
            if "REPLACE_L4_DPORT" == k and type(p4) in [UDP,TCP,SCTP]:
                ...
                upd_field_mask_num = 1
                config[k].append(0x10 + upd_field_mask_num)
                data = corsica_dpe.getEmActionUpdateField(v, layer, info)
